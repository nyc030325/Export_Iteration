...
theorem Nesterov_second_convergence (minφ : IsMinOn (f + h) Set.univ xm):
  ...
  have hieq5 : ∀ k : ℕ+, f (alg.x k) ≤ f (alg.z k) + ⟪f' (alg.z k), alg.x k - alg.z k⟫
    ...
    . rw [nm0]
      simp
    . push_neg at nm0
      have htpos : 0 < alg.t k := (alg.tbound k).1
      have ht_le : alg.t k ≤ 1 / (alg.l : ℝ) := (alg.tbound k).2
      have hlnz : (alg.l : ℝ) ≠ 0 := ne_of_gt alg.hl
      have ht_mul : alg.t k * (alg.l : ℝ) ≤ 1 := by
        have htpos' : 0 ≤ (alg.l : ℝ) := by exact_mod_cast (alg.l.property)
        calc
          alg.t k * (alg.l : ℝ) ≤ (1 / (alg.l : ℝ)) * (alg.l : ℝ) :=
            mul_le_mul_of_nonneg_right ht_le htpos'
          _ = 1 := by field_simp [hlnz]
      have ht_mul' : (alg.l : ℝ) * alg.t k ≤ 1 := by simpa [mul_comm] using ht_mul
      have hlt_base : (alg.l : ℝ) ≤ 1 / (alg.t k) := (le_div_iff htpos).2 ht_mul'
      have hlt' : (alg.l : ℝ) / 2 ≤ 1 / (2 * alg.t k) := by
        have hhalf : 0 ≤ (1 / (2 : ℝ)) := by norm_num
        have := mul_le_mul_of_nonneg_right hlt_base hhalf
        have hLeft : (alg.l : ℝ) * (1 / 2) = (alg.l : ℝ) / 2 := by ring
        have hRight : (1 / alg.t k) * (1 / 2) = 1 / (2 * alg.t k) := by field_simp; ring
        simpa [hLeft, hRight, mul_comm, mul_left_comm, mul_assoc] using this
      have hnonneg : 0 ≤ ‖alg.x ↑k - alg.z k‖ ^ 2 := by nlinarith
      have hmul := mul_le_mul_of_nonneg_right hlt' hnonneg
